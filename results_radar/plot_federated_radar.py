#!/usr/bin/env python3
"""Radar chart per federated method, each axis = a metric averaged over the 4
NetFlow datasets. Mirror of plot_centralised_radar.py for the federated runs, so
the two figures are directly comparable. Recomputes averages from
results_infer/*/metrics.json. FPR is shown as Specificity (1-FPR) so that outward
= better on every axis. Writes results_radar/federated_radar.png (per-method,
dataset-averaged) and results_radar/federated_radar_by_dataset.png (per-dataset
breakdown, 3 method polygons)."""
import json
import glob
import os
import collections
...
CONFIGS = {
    "FedAvg":                    "results_infer/results_examples/seed_*/federated",
    "FedRep":                    "results_infer/fed_rep/results_equal_norm/seed_*/federated",
    "FedAvg + finetune decoder": "results_infer/federated_finetune/seed_*/federated",
}
...
# categorical palette (dataviz skill, fixed order)
DS_COLORS = ["#2a78d6", "#1baf7a", "#eda100", "#008300"]  # CICIDS, UNSW, BoT, ToN
...
def main():
    ...
    # ---- Figure 1: per-method, dataset-averaged (mirrors centralised) -------
    ACCENT, BEST = "#2a78d6", "#008300"
    ncol = len(averages)
    fig, axes = plt.subplots(1, ncol, figsize=(4.2 * ncol, 5.3),
    ...
    out1 = os.path.join(HERE, "federated_radar.png")
    fig.savefig(out1, dpi=150, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"wrote {out1}")

    # ---- Figure 2: per-dataset breakdown, 3 method polygons -----------------
    fig, axes = plt.subplots(1, 4, figsize=(4.2 * 4, 5.6), subplot_kw=dict(polar=True))
    axes = axes.ravel()
    METHOD_COLORS = ["#2a78d6", "#eb6834", "#4a3aa7"]  # blue, orange, violet
    ...
    out2 = os.path.join(HERE, "federated_radar_by_dataset.png")
